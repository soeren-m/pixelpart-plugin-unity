...
def options(opts):
    opts.Add("android_api_level",
        "Target Android API level", "21")
    opts.Add("ndk_version",
        "Fully qualified version of ndk to use for compilation", "23.2.8568313")
    opts.Add("ANDROID_HOME",
        "Path to Android SDK installation, uses ANDROID_HOME from defined environment variables by default",
...
def generate(env):
    ...
    if int(env["android_api_level"]) < 21:
        print("Minimum supported Android target api is 21. Forcing target api 21.")
    ...
    if not os.path.exists(toolchain):
        print("Could not find NDK toolchain at " + toolchain + ".")
